...
#ifndef ESPAsyncWiFiManagerCustom_h
#define ESPAsyncWiFiManagerCustom_h
...
// fix crash on ESP32 (see https://github.com/alanswx/ESPAsyncWiFiManagerCustom/issues/44)
#if defined(ESP8266)
...
class AsyncWiFiManagerCustomParameter
{
public:
  AsyncWiFiManagerCustomParameter(const char *custom);
  AsyncWiFiManagerCustomParameter(const char *id,
...
private:
  ...
  friend class AsyncWiFiManagerCustom;
};
...
class AsyncWiFiManagerCustom
{
public:
#ifdef USE_EADNS
  AsyncWiFiManagerCustom(AsyncWebServer *server, AsyncDNSServer *dns);
#else
  AsyncWiFiManagerCustom(AsyncWebServer *server, DNSServer *dns);
#endif
  ...
  void setAPCallback(std::function<void(AsyncWiFiManagerCustom *)>);
  // called when settings have been changed and connection was successful
  void setSaveConfigCallback(std::function<void()> func);
  //adds a custom parameter
  void addParameter(AsyncWiFiManagerCustomParameter *p);
...
private:
  ...
  std::function<void(AsyncWiFiManagerCustom *)> _apcallback;
  std::function<void()> _savecallback;

  AsyncWiFiManagerCustomParameter *_params[WIFI_MANAGER_MAX_PARAMS];
