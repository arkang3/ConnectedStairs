/**************************************************************
   AsyncWiFiManagerCustom is a library for the ESP8266/Arduino platform
...
AsyncWiFiManagerCustomParameter::AsyncWiFiManagerCustomParameter(const char *custom)
{
  _id = NULL;
  _placeholder = NULL;
  _length = 0;
...
AsyncWiFiManagerCustomParameter::AsyncWiFiManagerCustomParameter(const char *id,
...
void AsyncWiFiManagerCustomParameter::init(const char *id,
                                     const char *placeholder,
                                     const char *defaultValue,
                                     unsigned int length,
                                     const char *custom)
...
const char *AsyncWiFiManagerCustomParameter::getValue()
{
  return _value;
}
const char *AsyncWiFiManagerCustomParameter::getID()
{
  return _id;
}
const char *AsyncWiFiManagerCustomParameter::getPlaceholder()
{
  return _placeholder;
}
unsigned int AsyncWiFiManagerCustomParameter::getValueLength()
{
  return _length;
}
const char *AsyncWiFiManagerCustomParameter::getCustomHTML()
{
  return _customHTML;
}

#ifdef USE_EADNS
AsyncWiFiManagerCustom::AsyncWiFiManagerCustom(AsyncWebServer *server,
...
void AsyncWiFiManagerCustom::addParameter(AsyncWiFiManagerCustomParameter *p)
{
...
void AsyncWiFiManagerCustom::setupConfigPortal()
{
...
// setup the DNS server redirecting all the domains to the apIP
  ...
  // server->on("/",
  //            std::bind(&AsyncWiFiManagerCustom::handleRoot, this, std::placeholders::_1))
  //     .setFilter(ON_AP_FILTER);
  server->on("/wifi",
             std::bind(&AsyncWiFiManagerCustom::handleWifi, this, std::placeholders::_1, true))
      .setFilter(ON_AP_FILTER);
  server->on("/0wifi",
             std::bind(&AsyncWiFiManagerCustom::handleWifi, this, std::placeholders::_1, false))
      .setFilter(ON_AP_FILTER);
  server->on("/wifisave",
             std::bind(&AsyncWiFiManagerCustom::handleWifiSave, this, std::placeholders::_1))
      .setFilter(ON_AP_FILTER);
  server->on("/i",
             std::bind(&AsyncWiFiManagerCustom::handleInfo, this, std::placeholders::_1))
      .setFilter(ON_AP_FILTER);
  server->on("/r",
             std::bind(&AsyncWiFiManagerCustom::handleReset, this, std::placeholders::_1))
      .setFilter(ON_AP_FILTER);
  server->on("/fwlink",
             std::bind(&AsyncWiFiManagerCustom::handleRoot, this, std::placeholders::_1))
      .setFilter(ON_AP_FILTER); // Microsoft captive portal. Maybe not needed. Might be handled by notFound handler.
  server->onNotFound(std::bind(&AsyncWiFiManagerCustom::handleNotFound, this, std::placeholders::_1));
...
boolean AsyncWiFiManagerCustom::autoConnect(unsigned long maxConnectRetries,
...
boolean AsyncWiFiManagerCustom::autoConnect(char const *apName,
...
String AsyncWiFiManagerCustom::networkListAsString()
{
  String pager;
  ...
  for (int i = 0; i < wifiSSIDCount; i++)
  {
    ...
    if (_minimumQuality == 0 || _minimumQuality < quality)
    {
      ...
      String rssiQ;
      rssiQ += quality;
      String item = "<option value=\""+wifiSSIDs[i].SSID+"\">"+wifiSSIDs[i].SSID+" "+rssiQ+"</option>";
...
String AsyncWiFiManagerCustom::scanModal()
{
  shouldscan = true;
  scan();
...
void AsyncWiFiManagerCustom::scan(boolean async)
{
  if (!shouldscan)
  {
    return;
...
void AsyncWiFiManagerCustom::copySSIDInfo(wifi_ssid_count_t n)
{
...
void AsyncWiFiManagerCustom::startConfigPortalModeless(char const *apName, char const *apPassword)
{
  _modeless = true;
  _apName = apName;
...
void AsyncWiFiManagerCustom::loop()
{
  safeLoop();
  criticalLoop();
}

void AsyncWiFiManagerCustom::setInfo()
{
  if (needInfo)
  {
    pager = infoAsString();
...
void AsyncWiFiManagerCustom::criticalLoop()
{
  if (_modeless)
  {
...
void AsyncWiFiManagerCustom::safeLoop()
{
#ifndef USE_EADNS
...
boolean AsyncWiFiManagerCustom::startConfigPortal(char const *apName, char const *apPassword)
{
  // setup AP
  WiFi.mode(WIFI_AP_STA);
  DEBUG_WM(F("SET AP STA"));
...
uint8_t AsyncWiFiManagerCustom::connectWifi(String ssid, String pass)
{
...
uint8_t AsyncWiFiManagerCustom::waitForConnectResult()
{
...
#ifdef NO_EXTRA_4K_HEAP
void AsyncWiFiManagerCustom::startWPS()
{
...
#endif
String AsyncWiFiManagerCustom::getConfigPortalSSID()
{
  return _apName;
}

void AsyncWiFiManagerCustom::resetSettings()
{
...
void AsyncWiFiManagerCustom::setTimeout(unsigned long seconds)
{
  setConfigPortalTimeout(seconds);
}

void AsyncWiFiManagerCustom::setConfigPortalTimeout(unsigned long seconds)
{
  _configPortalTimeout = seconds * 1000;
}

void AsyncWiFiManagerCustom::setConnectTimeout(unsigned long seconds)
{
  _connectTimeout = seconds * 1000;
}

void AsyncWiFiManagerCustom::setTryConnectDuringConfigPortal(boolean v)
{
  _tryConnectDuringConfigPortal = v;
}

void AsyncWiFiManagerCustom::setDebugOutput(boolean debug)
{
  _debug = debug;
}

void AsyncWiFiManagerCustom::setAPStaticIPConfig(IPAddress ip,
                                           IPAddress gw,
                                           IPAddress sn)
{
  _ap_static_ip = ip;
...
void AsyncWiFiManagerCustom::setSTAStaticIPConfig(IPAddress ip,
                                            IPAddress gw,
                                            IPAddress sn,
                                            IPAddress dns1,
                                            IPAddress dns2)
...
void AsyncWiFiManagerCustom::setMinimumSignalQuality(unsigned int quality)
{
  _minimumQuality = quality;
}

void AsyncWiFiManagerCustom::setBreakAfterConfig(boolean shouldBreak)
{
  _shouldBreakAfterConfig = shouldBreak;
}

// handle root or redirect to captive portal
void AsyncWiFiManagerCustom::handleRoot(AsyncWebServerRequest *request)
{
  ...
  // page += _apName;
  // page += "</h1>";
  // page += F("<h3>AsyncWiFiManagerCustom</h3>");
...
void AsyncWiFiManagerCustom::handleWifi(AsyncWebServerRequest *request, boolean scan)
{
  shouldscan = true;
  scannow = 0;
  ...
  if (scan)
  {
    ...
    {
      // display networks in page
      pager = networkListAsString();
      Serial.print("pager :");Serial.println(pager);
  ...
  request->send(LittleFS, "/wifi.html", "text/html",false,[&pager](const String& var){


      if(var == "SSID_LIST"){
        Serial.print("ssdlist :");Serial.println(pager);
        return pager;
      }
      return String();
  });
...
void AsyncWiFiManagerCustom::handleWifiSave(AsyncWebServerRequest *request)
{
...
String AsyncWiFiManagerCustom::infoAsString()
{
  String page;
...
void AsyncWiFiManagerCustom::handleInfo(AsyncWebServerRequest *request)
{
  DEBUG_WM(F("Info"));
...
void AsyncWiFiManagerCustom::handleReset(AsyncWebServerRequest *request)
{
  DEBUG_WM(F("Reset"));
...
void AsyncWiFiManagerCustom::handleNotFound(AsyncWebServerRequest *request)
{
...
boolean AsyncWiFiManagerCustom::captivePortal(AsyncWebServerRequest *request)
{
...
void AsyncWiFiManagerCustom::setAPCallback(std::function<void(AsyncWiFiManagerCustom *)> func)
{
  _apcallback = func;
}

// start up save config callback
void AsyncWiFiManagerCustom::setSaveConfigCallback(std::function<void()> func)
{
  _savecallback = func;
}

// sets a custom element to add to head, like a new style tag
void AsyncWiFiManagerCustom::setCustomHeadElement(const char *element)
{
  _customHeadElement = element;
}

// sets a custom element to add to options page
void AsyncWiFiManagerCustom::setCustomOptionsElement(const char *element)
{
  _customOptionsElement = element;
}

// if this is true, remove duplicated Access Points - defaut true
void AsyncWiFiManagerCustom::setRemoveDuplicateAPs(boolean removeDuplicates)
{
  _removeDuplicateAPs = removeDuplicates;
}

template <typename Generic>
void AsyncWiFiManagerCustom::DEBUG_WM(Generic text)
{
  if (_debug)
  {
    Serial.print(F("*WM: "));
...
unsigned int AsyncWiFiManagerCustom::getRSSIasQuality(int RSSI)
{
...
// is this an IP?
boolean AsyncWiFiManagerCustom::isIp(String str)
{
...
// IP to String?
String AsyncWiFiManagerCustom::toStringIp(IPAddress ip)
{
  String res = "";
  for (int i = 0; i < 3; i++)
  {
